...
[![Version](https://img.shields.io/badge/Version-v1.0.0-4c6ef5.svg)](/)
...
节点归属统一由 `payload.domain` 字段表达（v1.0.0 起；历史字段 `character_name` 已退役）。写入记忆时通过 `--domain X` 或 `mem_ingest(domain=...)` 指定区块；`kb` 类型节点由知识库索引自动设置（`kb` / `rule`）。
