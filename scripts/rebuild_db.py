"""[一次性脚本 · 勿复用]
=====================================================================
仅用于 2026-08-28 那次数据重建，不是通用工具：EXPORT_PATH /
EXPECTED_NODES / EXPECTED_EDGES 都是当次导出文件的硬编码值，
换一份导出或换一个库必然校验失败。

日常用途请改用：
  - 向量重嵌入：scripts/reindex.py
  - 域字段迁移：scripts/migrate_domain.py
  - 知识库重建：scripts/build_kb_index.py

（下方正文描述的是更早一次 2026-08-24 局部重建的口径——只恢复 67 个
非 kb_chunk 节点；当前常量已改为 2026-08-28 全量恢复 391 节点，
冲突处以常量与代码为准。）
---------------------------------------------------------------------
Palimpsest 数据库重建脚本
========================
...
def restore(db, store: TriviumStore, nodes: list, edges: list) -> dict:
    """在已打开的句柄上恢复全部节点（含 kb_chunk）+ 可映射边，返回 {旧id: 新id}。"""
    restore_nodes = nodes
    print(f"恢复节点: {len(restore_nodes)}（全量恢复，含 kb_chunk）")
